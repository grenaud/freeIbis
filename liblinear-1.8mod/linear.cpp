...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	if(model_->nr_class == 2 ){
		...
		for(i=0; i<(w_size); i++){ //for each feature
		    int j;
		    for(j=0; j<nr_w; j++){ //for each class
			fprintf(fp, "%.16g ", -1.0*model_->w[i*nr_w+j]);
		    }
		    fprintf(fp, "\n");
		}
		fprintf(fp, "%.16g\n", model_->bias);
		...
		if(model_->label[0] == 1 && 
		   model_->label[1] == 0){
		    ...
		    printf("Wrong labelling for binary mode\n");
		    return -1;
		}
	    ...
	    for(i=0; i<(w_size-1); i++){ //for each feature
		int j;
		for(j=0; j<nr_w; j++){ //for each class
		    int correctIndex;
		    //find correct index, inefficient
		    for(int x=0; x<model_->nr_class; x++)
			if(model_->label[x] == (j+1))
			    correctIndex=x;
		    fprintf(fp, "%.16g ", model_->w[i*nr_w+correctIndex]);
		}
		fprintf(fp, "\n");
	    }
	}
