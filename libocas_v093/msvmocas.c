  ...
  if(mxIsSparse(data_X))
  {
    ...
    if(verb)
    {
      ...
      ocas = msvm_ocas_solver( C, data_y, nY, nData, TolRel, TolAbs, QPBound, MaxTime,BufSize, Method,
                               ...
                               &msvm_sparse_compute_output, &qsort_data, &ocas_print,&sumw, 0);
    }
    else
    {
      ocas = msvm_ocas_solver( C, data_y, nY, nData, TolRel, TolAbs, QPBound, MaxTime,BufSize, Method,
                               &msvm_sparse_compute_W, &msvm_full_update_W, &msvm_sparse_add_new_cut,
                               &msvm_sparse_compute_output, &qsort_data, &ocas_print_null,&sumw, 0);
    }
  }
  else
  {
    ...
    if(verb)
    {
      ...
      ocas = msvm_ocas_solver( C, data_y, nY, nData, TolRel, TolAbs, QPBound, MaxTime,BufSize, Method,
                               ...
                               &msvm_full_compute_output, &qsort_data, &ocas_print, &sumw,0); 
    }
    else
    {
      ocas = msvm_ocas_solver( C, data_y, nY, nData, TolRel, TolAbs, QPBound, MaxTime,BufSize, Method,
                               &msvm_full_compute_W, &msvm_full_update_W, &msvm_full_add_new_cut,
                               &msvm_full_compute_output, &qsort_data, &ocas_print_null, &sumw,0); 
