...
double sumw(){  
    ...
    for(i=0; i < nDim; i++) {
	for(j=0; j < nY; j++){
	    sprintf(arraychars, "%.20f ", W[LIBOCAS_INDEX(i,j,nDim)]);    
	    temp=atof(arraychars);
	    if(temp > 0.0)
		sumwToReturn+=log(temp);
	}
    }
...
/*  mexPrintf("Input file: %s\n", fname);*/
  ...
  while(go) {
    ...
    else
    {
      ...
      if(nnzf == -1) {
	  mexPrintf("Parsing error on line #%ld line =%s# file=%s#\n", line_cnt,line,fname);
